...
const SectionWhatWeDo = () => {
  const { dataApi } = useFetch(`what-we-do`);
      ...
      <div className='container mx-auto lg:w-10/12 md:w-11/12 px-4 py-24 md:my-5'>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2  md:gap-y-10 gap-x-10'>
          ...
          <div className='flex justify-end items-center '>
            <img className='w-full md:w-full lg:w-auto h-[490px] object-contain mb-[-40px] md:mb-4' src={whatWeDo && whatWeDo.imageUrl} alt='' />
          ...
          <div>
            <div className='flex flex-col w-ful'>
              {/* About Us heading */}
              <h1 className={`text-accent text-xl py-1 flex lg:text-[24px] md:text-[20px] text-left font-bold`}>\ What We Do \</h1>

              {/* Main title with responsive text size */}
              <p className={`text-primary lg:text-[2.5rem] md:text-[2rem] text-[1.5rem] py-1 font-semibold text-left`}>{whatWeDo && whatWeDo.title}</p>
            </div>

            {/* Description text */}
            <p className='text-base lg:pr-32 mb-8 py-5'>{whatWeDo && whatWeDo.desc}</p>
