...
const SectionContact = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData({
      ...
      [name]: DOMPurify.sanitize(value), // Update the value of the field being changed
    });
  };
    ...
    <section className="bg-primary py-32 font-raleway">
      ...
      <div className="flex justify-center mb-20">
        <div className="flex flex-col w-full text-center justify-center items-center">
          {/* Title and subtitle */}
          <h1 className="text-accent text-xl py-1 flex lg:text-[24px] md:text-[20px] text-center font-bold">
            \ Get In Touch \
          </h1>
          <p className="text-white lg:text-[2.5rem] md:text-[2rem] text-[1.5rem] py-1 font-semibold text-left">
            Hey! Let’s Talk
          </p>
        </div>
      </div>
          ...
          <div className="col-span-2 border p-8 rounded-xl bg-white">
            <form onSubmit={handleSubmit}>
              ...
              <div className="mt-4">
                <button
                  type="submit"
                  className="w-32 py-3 px-4 text-sm font-medium rounded-lg bg-accent text-white hover:bg-blue-700"
                >
                  Send Now
                </button>
              </div>
          ...
          <div>
            <div className="p-8 bg-white rounded-xl">
              <div className="flex flex-col gap-y-[30px]">
                ...
                {contactCardData.map((item, index) => (
                  <div key={index} className="flex gap-4">
                    ...
                    <div>
                      <h3 className="text-accent font-semibold text-[20px]">
                        {item.title}
                      </h3>
              ...
              <div className="mt-[50px]">
                <h3 className="text-primary text-[24px] font-semibold text-center">
                  Follow Us
                </h3>
                <div className="flex gap-4 mt-4 justify-center text-white text-2xl">
                  ...
                  {contactCardSosial.map((item, index) => (
                    <div
                      key={index}
                      className="border px-4 py-3 rounded-md bg-accent"
                    >
